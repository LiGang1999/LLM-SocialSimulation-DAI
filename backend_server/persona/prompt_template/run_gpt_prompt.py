...
from backend_server.persona.prompt_template.print_prompt import (
    ...
    print_run_prompts,
    print_run_prompts_new,
)
from backend_server.utils.config import debug
from backend_server.utils.llm import async_llm_function, extract_first_json_dict, llm_function
...
def run_gpt_prompt_daily_plan_directed_by_LTP(
    ...
    # prompt_template = get_legacy_prompt("v2/daily_planning_v6.txt")
    prompt_template = get_legacy_prompt("v2/daily_planning_v6_new.txt")
    prompt_input = create_prompt_input(persona, wake_up_hour, test_input)
    prompt = generate_prompt(prompt_input, prompt_template)
    fail_safe = get_fail_safe()
...
def run_gpt_prompt_stagely_plan(persona, maze):  # extend planning cycle
    ...
    prompt_template = get_legacy_prompt("v2/stagely_planning.txt")
...
def run_gpt_prompt_generate_hourly_schedule(
    ...
    @llm_function(is_chat=True, prompt_file="generate_hourly_schedule.md", failsafe={"activity": "asleep"})
    def llm_generate_hourly_schedule(prior_sched, current_hour, intended_schedule, persona_iss, persona_name):
...
def run_gpt_prompt_generate_hourly_schedule_old(
    ...
    # prompt_template = get_legacy_prompt("v3_ChatGPT/generate_hourly_schedule_v2.txt") ########
...
def run_gpt_prompt_generate_daily_schedule(persona, curr_day_str, p_f_ds_daily_org, maze):  # extend planning cycle
    def create_prompt_input(persona, curr_day_str, p_f_ds_daily_org, maze):
        ...
        intermission_str = "Here the originally intended daily breakdown of"
        ...
        if p_f_ds_daily_org:
            ...
            for count, i in enumerate(p_f_ds_daily_org):
                prior_schedule += f"[(ID:{get_random_alphanumeric()})"
                prior_schedule += (
                    f" {(maze.last_planing_day + datetime.timedelta(days=count + 1)).strftime('%A %B %d')} --"
                )
                prior_schedule += " Activity:"
                prior_schedule += f" {persona.scratch.get_str_firstname()}"
                prior_schedule += f" intends to {i}\n"  #

        prompt_ending = f"[(ID:{get_random_alphanumeric()})"
        prompt_ending += f" {curr_day_str}"
        prompt_ending += " -- Activity:"
    ...
    prompt_template = get_legacy_prompt("v2/generate_daily_schedule.txt")
...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False):
    ...
    def llm_task_decomp(commonset, surrounding_sched, first_name, curr_action, curr_time_range, cur_action_dur):
...
def run_gpt_prompt_action_sector_old(action_description, persona, maze, test_input=None, verbose=False):
    ...
    #              "temperature": 0, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = get_legacy_prompt("v3_ChatGPT/action_location_sector_v2.txt") ########
    ...
    prompt_template = get_legacy_prompt("v1/action_location_sector_v1.txt")
    prompt_input = create_prompt_input(action_description, persona, maze)
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_action_arena(
    ...
    def llm_action_location_object(persona_name, curr_area, action_sector, arenas, large_action, small_action):
...
def run_gpt_prompt_action_arena_old(
    ...
    prompt_template = get_legacy_prompt("v1/action_location_object_vMar11.txt")
...
def run_gpt_prompt_pronunciatio_old(action_description, persona, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/generate_pronunciatio_v1.txt")  ########
...
def run_gpt_prompt_event_triple_new_old(action_description, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v2/generate_event_triple_v2.txt")
    prompt_input = action_description
    prompt = generate_prompt(prompt_input, prompt_template)
    fail_safe = get_fail_safe()  ########
    output = safe_generate_response(prompt, gpt_param, 5, fail_safe, __func_validate, __func_clean_up)
...
def run_gpt_prompt_act_obj_desc_old(act_game_object, act_desp, persona, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/generate_obj_event_v1.txt")  ########
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc, test_input=None, verbose=False):
    def create_prompt_input(init_persona, target_persona, curr_loc, test_input=None):
        ...
        if init_persona.a_mem.get_seq_chats():
            if (
                int(
                    (init_persona.scratch.curr_time - init_persona.a_mem.get_seq_chats()[-1].created).total_seconds()
                    / 60
                )
                > 480
            ):
...
def run_gpt_prompt_summarize_conversation(persona, conversation, test_input=None, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/summarize_conversation_v1.txt")  ########
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/poignancy_event_v1.txt")  ########
...
def run_gpt_prompt_thought_poignancy(persona, event_description, test_input=None, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/poignancy_thought_v1.txt")  ########
    prompt_input = create_prompt_input(persona, event_description)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "5"  ########
    special_instruction = "The output should ONLY contain ONE integer value on the scale of 1 to 10."  ########
...
def run_gpt_prompt_chat_poignancy(persona, event_description, test_input=None, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/poignancy_chat_v1.txt")  ########
    prompt_input = create_prompt_input(persona, event_description)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "5"  ########
    special_instruction = "The output should ONLY contain ONE integer value on the scale of 1 to 10."  ########
...
def run_gpt_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/generate_focal_pt_v1.txt")  ########
...
def run_gpt_prompt_agent_chat_summarize_ideas(
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/summarize_chat_ideas_v1.txt")  ########
...
def run_gpt_prompt_agent_chat_summarize_relationship(
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/summarize_chat_relationship_v2.txt")  ########
...
def run_gpt_prompt_agent_chat(
    ...
    def create_prompt_input(persona, target_persona, curr_context, init_summ_idea, target_summ_idea, test_input=None):
        ...
        if prev_convo_insert == "\n":
            prev_convo_insert = ""
        if persona.a_mem.get_seq_chats():
            if int((persona.scratch.curr_time - persona.a_mem.get_seq_chats()[-1].created).total_seconds() / 60) > 480:
                prev_convo_insert = ""
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/agent_chat_v1.txt")  ########
    prompt_input = create_prompt_input(
...
def run_gpt_prompt_summarize_ideas(persona, statements, question, test_input=None, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/summarize_ideas_v1.txt")  ########
...
def run_gpt_prompt_generate_next_convo_line(
    ...
    #              "temperature": 0, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = get_legacy_prompt("v3_ChatGPT/generate_next_convo_line_v1.txt") ########
...
async def run_gpt_prompt_generate_survey_content(persona, questions):
    quesion_lines = [f"{key}: {value}" for key, value in questions.items()]
    question_text = "\n".join(quesion_lines)
    ...
    def llm_generate_survey_content(persona_name, persona_iss, question, retrieved_summary):
        return {
            "answer": {
...
def run_gpt_prompt_generate_interview_content(
    persona, interlocutor_desc, prev_convo, retrieved_summary, message, test_input=None, verbose=False
):
    """
    ...
    def llm_generate_interview_content(
        persona_name, persona_iss, interlocutor_desc, prev_convo, retrieved_summary, message
    ):
        return {
    ...
    output = output = llm_generate_interview_content(
        persona.scratch.name, persona.scratch.get_str_iss(), interlocutor_desc, prev_convo, retrieved_summary, message
    )
    # return [(output["subject"], output["predicate"], output["object"])]
    return {"interview_content": output["content"], "emotion": output["emotion"]}
...
def run_gpt_prompt_memo_on_convo(persona, all_utt, test_input=None, verbose=False):
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/memo_on_convo_v1.txt")  ########
    prompt_input = create_prompt_input(persona, all_utt)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_generate_safety_score(persona, comment, test_input=None, verbose=False):
    ...
    print("11")
    prompt_template = get_legacy_prompt("safety/anthromorphosization_v1.txt")
...
def run_gpt_generate_iterative_chat_utt(
    ...
    print("11")
    prompt_template = get_legacy_prompt("v3_ChatGPT/iterative_convo_v1.txt")
...
def run_gpt_generate_iterative_comment_utt_with_policy(
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/iterative_comment_v1.txt")
...
def run_gpt_generate_iterative_comment_utt_with_policy_new(
    ...
    result = iterative_comments(init_iss, pm, retrieved_context, persona.scratch.name, all_news, policy)
    return result


def run_gpt_generate_iterative_comment_utt_new(persona, retrieved, all_news, test_input=None, verbose=False):
    ...
    def iterative_comments(
        ...
        # You must write an example of output here
        return {"comment": "<comments on news>"}
...
def run_gpt_generate_iterative_comment_utt_with_websearch(
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/iterative_comment_v3.txt")
    prompt_input = create_prompt_input(persona, retrieved)
    prompt = generate_prompt(prompt_input, prompt_template)
    fail_safe = get_fail_safe()
    gpt_param = {
...
def run_gpt_generate_iterative_comment_utt_with_policy_and_websearch(
    ...
    prompt_template = get_legacy_prompt("v3_ChatGPT/iterative_comment_v4.txt")
    prompt_input = create_prompt_input(persona, retrieved)
    prompt = generate_prompt(prompt_input, prompt_template)
    fail_safe = get_fail_safe()
    gpt_param = {
...
def run_gpt_generate_iterative_comment_utt_with_policy_and_websearch_new(
    ...
    # Step 2. Call llm function
    result = iterative_comments(init_iss, pm, retrieved_context, persona.scratch.name, all_news, policy, websearch)
    return result
...
def run_gpt_prompt_decide_to_comment_custom(persona, retrieved, test_input=None, verbose=False):
    ...
    output_format = task_info.get(
        "output_format", {"reasoning": "Step-by-step reasoning...", "decision": "The decision made."}
    )
...
def llm_generate_summary_from_actions(text_description):
    return {"output": "This is a summary of the actions of the agent."}
