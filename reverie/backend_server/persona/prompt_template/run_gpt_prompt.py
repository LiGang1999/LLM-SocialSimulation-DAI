...
def run_gpt_prompt_daily_plan_directed_by_LTP(
    ...
    def create_prompt_input(
        persona, wake_up_hour, extraprompt=extraprompt, test_input=None
    ):
        if test_input:
            return test_input
        prompt_input = []
...
def run_gpt_prompt_stagely_plan(persona, maze):  # extend planning cycle
    def create_prompt_input(persona, maze):
        ...
        prompt_input += [persona.scratch.get_str_firstname()]
        prompt_input += [
            (maze.last_planning_day + datetime.timedelta(days=1)).strftime("%A %B %d")
        ]
        prompt_input += [
            (
                maze.last_planning_day + datetime.timedelta(days=maze.planning_cycle)
            ).strftime("%A %B %d")
        ]
        return prompt_input
...
def run_gpt_prompt_generate_hourly_schedule(
    ...
    @llm_function(
        is_chat=True,
        prompt_file="generate_hourly_schedule.md",
        failsafe={"activity": "asleep"},
    )
...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False):
    ...
    def llm_task_decomp(
        ...
        return [
            ...
            {
                "subtask": "creating materials for the lesson.",
                "duration": 30,
                "remaining": 75,
            },
            ...
            {
                "subtask": "putting the lesson plan in her bag.",
                "duration": 5,
                "remaining": 0,
            },
        ]
...
def run_gpt_prompt_task_decomp_old(
...
def run_gpt_prompt_action_game_object(
    ...
    def llm_action_object(action, objects):
        return {
            "object" : "the most relevant object"
        }

    output = llm_action_object(action=action_description, objects=persona.s_mem.get_str_accessible_arena_game_objects(temp_address))
...
def run_gpt_generate_iterative_comment_utt_new(
    ...
    result = iterative_comments(
        init_iss, pm, retrieved_context, persona.scratch.name, all_news
    )
    return result
...
###wzt plan里面调用的判断
def run_gpt_prompt_decide_to_comment(
    persona, retrieved, test_input=None, verbose=False
):
    def create_prompt_input(persona, retrieved, test_input=None):
        pm = ""
        retrieved_context = ""
        ...
        prompt_input = (pm, retrieved_context, curr_time, persona.name, init_iss)
        return prompt_input

    pm, retrieved_context, curr_time, persona_name, init_iss = create_prompt_input(
        persona, retrieved, test_input
    )

    @llm_function(prompt_file="decide_to_comment.md", is_chat=True, stop="---")
    def decide_to_comment(public_memory, time, context, persona_name):
    ...
    output = decide_to_comment(pm, curr_time, retrieved_context, persona_name)
