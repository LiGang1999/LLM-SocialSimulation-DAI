...
def generate_first_daily_plan_directed_by_LTP(
    ...
    if debug:
        print("GNS FUNCTION: <generate_first_daily_plan>")
    return run_gpt_prompt_daily_plan_directed_by_LTP(
        persona, wake_up_hour, extraprompt
    )[0]
...
def _long_term_planning(persona, new_day, maze):
    ...
    if new_day == "First day":
        ...
        if maze.planning_cycle == 1:
            ...
            for activity_oneday in persona.scratch.f_stagely_schedule:
                if persona.scratch.curr_time.strftime("%A %B %d") == activity_oneday[1]:
                    ...
                    break
            persona.scratch.daily_req = generate_first_daily_plan_directed_by_LTP(
                persona, wake_up_hour, extraprompt=activity
            )
        ...
        persona.scratch.daily_req = persona.scratch.daily_req  # lg: ???
...
def generate_daily_schedule(persona, maze):  # extend planning cycle
    ...
    for i in range(maze.planning_cycle):
        curr_day = (maze.last_planning_day + datetime.timedelta(days=i + 1)).strftime(
            "%A %B %d"
        )
        curr_activity = run_gpt_prompt_generate_daily_schedule(
            persona, curr_day, temp, maze
        )[0]
        temp += [curr_activity]
...
def plan(persona, maze, personas, new_day, retrieved):
    ...
    if maze.need_stagely_planning and maze.planning_cycle > 1:
        ...
        persona.scratch.f_stagely_schedule_daily_org = (
            persona.scratch.f_stagely_schedule[:]
        )
        ...
        thought = thought[:-1] + "."
        created = persona.scratch.curr_time
        expiration = persona.scratch.curr_time + datetime.timedelta(
            days=30 + maze.planning_cycle
        )
        s, p, o = (
            persona.scratch.name,
            "plan",
        ...
        maze.last_planning_day = maze.last_planning_day + datetime.timedelta(
            days=maze.planning_cycle
        )  # extend planning cycle
    ###extend planning cycle###
    ...
    if persona.scratch.act_check_finished():
        ...
        # lg: generate_task_decomp()
        # lg: run_gpt_prompt_task_decomp()
        # lg: generate_action_sector()
        # lg: run_gpt_prompt_action_sector()
        # lg: generate_action_arena()
        # lg: run_gpt_prompt_action_arena()
        # lg: generate_action_game_object()
        # lg: run_gpt_prompt_action_game_object()
        # lg: generate_action_pronunciatio()
        # lg: run_gpt_prompt_pronunciatio()
        # lg: generate_action_event_triple()
        # lg: run_gpt_prompt_event_triple()
        # lg: generate_act_obj_desc()
        # lg: run_gpt_prompt_act_obj_desc()
        # lg: generate_act_obj_event_triple()
        # lg: run_gpt_prompt_act_obj_event_triple()
    ...
    if focused_event:
        ...
        # lg: lets_talk()
        # lg: generate_decide_to_talk()
        # lg: run_gpt_prompt_decide_to_talk()
        # lg: lets_react()
        # lg: generate_decide_to_react()
        # lg: run_gpt_prompt_decide_to_react()
        ...
        if reaction_mode:
            ...
            if reaction_mode[:9] == "chat with":
                ...
                # lg: generate_convo()
                # lg: agent_chat_v2()
                # lg: generate_convo_summary()
                # lg: run_gpt_prompt_summarize_conversation()
                # lg: _create_react()
                # lg: generate_new_decomp_schedule()
                # lg: run_gpt_prompt_new_decomp_schedule()
