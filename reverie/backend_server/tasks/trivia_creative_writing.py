...
class TriviaCreativeWritingTask(Task):
    ...
    def get_input_prompt(self, idx: int, method: str, **kwargs) -> str:
        ...
        if method == "standard":
            input_prompt = standard_prompt.format(
                n=n, questions=questions_str, topic=topic
            )
        elif method == "cot":
            ...
            input_prompt = spp_prompt_fixed_persona.format(
                n=n, questions=questions_str, topic=topic
            )
        elif method == "spp_profile":
            input_prompt = spp_prompt_profile.format(
                n=n, questions=questions_str, topic=topic
            )
        else:
