...
class CodenamesCollaborativeTask(Task):
    ...
    def get_input_prompt(self, idx: int, method: str, **kwargs) -> str:
        ...
        if role == "spymaster":
            if method == "standard":
                input_prompt = standard_prompt_spymaster.format(
                    n=n, target_words=target_words_str, word_list=word_list_str
                )
            elif method == "cot":
                input_prompt = cot_prompt_spymaster.format(
                    n=n, target_words=target_words_str, word_list=word_list_str
                )
            elif method == "spp":
                input_prompt = spp_prompt_spymaster.format(
                    n=n, target_words=target_words_str, word_list=word_list_str
                )
            elif method == "spp_fixed_persona":
                input_prompt = spp_prompt_spymaster_fixed_persona.format(
                    n=n, target_words=target_words_str, word_list=word_list_str
                )
            elif method == "spp_profile":
                input_prompt = spp_prompt_spymaster_profile.format(
                    n=n, target_words=target_words_str, word_list=word_list_str
                )
            else:
                raise NotImplementedError(
                    f"method {method} not implemented for spymaster role"
                )
        elif role == "guesser":
            if method == "standard":
                input_prompt = standard_prompt_guesser.format(
                    n=n, hint_word=hint_word, word_list=word_list_str
                )
            elif method == "cot":
                input_prompt = cot_prompt_guesser.format(
                    n=n, hint_word=hint_word, word_list=word_list_str
                )
            elif method == "spp":
                input_prompt = spp_prompt_guesser.format(
                    n=n, hint_word=hint_word, word_list=word_list_str
                )
            elif method == "spp_fixed_persona":
                input_prompt = spp_prompt_guesser_fixed_persona.format(
                    n=n, hint_word=hint_word, word_list=word_list_str
                )
            elif method == "spp_profile":
                input_prompt = spp_prompt_guesser_profile.format(
                    n=n, hint_word=hint_word, word_list=word_list_str
                )
            else:
                raise NotImplementedError(
                    f"method {method} not implemented for guesser role"
                )
        else:
            raise NotImplementedError(
                f"role {role} not implemented; choose from 'spymaster' or 'guesser'"
            )
        return input_prompt
    ...
    def test_output(self, idx: int, output: str):
        ...
        predicted_words = [
            word.strip().replace(".", "").lower() for word in predicted_words
        ]
