...
class DaiInstitution(Institution):
    ...
    def _run_task(self, task_name, gpt, task, i, method, num_generation, args):#
        if task_name in ['trivia_creative_writing', 'logic_grid_puzzle']:
            ...
            unwrapped_output_batch, if_success_batch = self._post_process_raw_response(task, raw_output_batch, method)
        ...
        # get prompt
            ...
            if raw_spymaster_output == [] or raw_response_spymaster == []: # handle exception
                return {}
            spymaster_output, if_success_batch_spymaster = self._post_process_raw_response(task, raw_spymaster_output, method)
            ...
            if raw_guesser_output == [] or raw_response_batch_guesser == []: # handle exception
                return {}
            guesser_output_batch, if_success_batch_guesser = self._post_process_raw_response(task, raw_guesser_output, method)
            # compute automatic metric (different for each task), e.g., if the output contains all the answers
    ...
    def run(self, case):
        ...
        if case is not None:
            ...
            log_output = self._run_task("institution", gpt, task, 0, method, num_generation, self.args)#
        ...
        for i in range(start, end):
            log_output = self._run_task(task_name, gpt, task, i, method, num_generation, self.args)#
